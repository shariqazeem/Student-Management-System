...
def fetch_data():
    ...
    if len(data) != 0:
        ...
        for i in data:
            teacher_table.insert("", END, values=i)
        connection.commit()
    connection.close()
...
teacher_table = ttk.Treeview(table_frame, column=("teacher_id",
                                                  "teacher_name",
                                                  "teacher_email",
                                                  "teacher_mobile",
...
scroll_bar_x.config(command=teacher_table.xview)
scroll_bar_y.config(command=teacher_table.yview)
...
teacher_table.heading("teacher_mobile", text="Teacher Mobile")
teacher_table.heading("teacher_qualification", text="Teacher Qualification")
teacher_table.heading("teaching_subject", text="Teaching Subject")
teacher_table.heading("course_mentor", text="Course Mentor")
teacher_table.heading("monthly_salary", text="monthly Salary")
teacher_table.heading("gender", text="Gender")
teacher_table.heading("age", text="age")
teacher_table.heading("residential_address", text="Residential Address")

teacher_table["show"] = "headings"

teacher_table.column("teacher_id", width=100)
teacher_table.column("teacher_name", width=150)
teacher_table.column("teacher_email", width=200)
teacher_table.column("teacher_mobile", width=150)
teacher_table.column("teacher_qualification", width=200)
teacher_table.column("teaching_subject", width=200)
teacher_table.column("course_mentor", width=200)
teacher_table.column("monthly_salary", width=100)
teacher_table.column("gender", width=100)
teacher_table.column("age", width=100)
teacher_table.column("residential_address", width=500)

teacher_table.pack(fill=BOTH, expand=1)
teacher_table.bind("<ButtonRelease>", get_cursor)
fetch_data()
